...
links2={}
read_file=open("zhengzhuangbaike.txt","a")
readall_file=open("zhengzhuangbaikedetial.txt","a")
webdata={}
firstURL="http://weixin.myquanwei.com/wx/sympTwo"
readed_file = open("zhengzhuangbaike.txt","r")
...
try:
        ...
        if cateLogpage:
                ...
                data=catalogPageTxt.split("<div class=\"wrap_box\">")[1]
                links=data.split("<li onclick=\"javascript:location.href=\'")
                for j in range(1,len(links)):
                        print "j"
                        print j                         
                        data2=links[j].split("\';\"")[0]
                        name=links[j].split("</li>")[0].split(">")[1]
                        linkaddr="http://weixin.myquanwei.com"+data2
                        print linkaddr
                        webdata[linkaddr]=name
                        read_file.write(linkaddr+" "+name+"\n")
                        read_file.flush()            
        pass
print ""
print "detial start:"
...
readall_file.write("病名,地址,简介,病因,症状特点,对症处理,预防护理,饮食宜忌,可能疾病\n".decode('utf-8').encode(sys.getfilesystemencoding()))
readall_file.flush()
...
for weblink in webdata.keys():
        ...
        try:   
                ...
                if detialPage:
                        ...
                        readall_file.write((bingming+","+dizhi+","+brief+","+bingyin+","+zhengzhuangtedian+","+duizhengchuli+","+yufanghuli+","+yinshijinji+","+kenengjibing+"\n").decode('utf-8').encode(sys.getfilesystemencoding()))
                        readall_file.flush()
